...
const AntigravityInner = ({
  ...
  useFrame(state => {
    ...
    particles.forEach((particle, i) => {
      ...
      const proj = 1 - cz / 50;
      const ptx = targetX * proj, pty = targetY * proj;
      const dx = mx - ptx, dy = my - pty;
      ...
      if (dist < magnetRadius) {
        ...
        targetPos = { x: ptx + r * Math.cos(angle), y: pty + r * Math.sin(angle), z: mz * depthFactor + Math.sin(t) * waveAmplitude * depthFactor };
      }
      ...
      const d = Math.hypot(particle.cx - targetX, particle.cy - targetY);
      const sf = Math.max(0, Math.min(1, 1 - Math.abs(d - ringRadius) / 10));
      const fs = sf * (0.8 + Math.sin(t * pulseSpeed) * 0.2 * particleVariance) * particleSize;
