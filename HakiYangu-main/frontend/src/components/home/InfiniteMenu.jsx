...
function resizeCanvasToDisplaySize(canvas) {
  ...
  return canvas.width !== dw || canvas.height !== dh;
}
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (vec2.sqrLen(midPointerPos) > this.EPSILON) {
        ...
        const p = this._project(midPointerPos), q = this._project(this.previousPointerPos);
...
class InfiniteGridMenu {
  ...
  _init(onInit) {
    ...
    const gl = this.gl;
    if (!gl) { console.error('No WebGL 2 context!'); return; }
  ...
  _initTexture() {
    ...
    const PLACEHOLDER_COLORS = ['#DB1A1A', '#D4AF37', '#16a34a', '#6366f1', '#DB4D4D'];
    ...
    const drawPlaceholders = () => {
      for (let i = 0; i < itemCount; i++) {
        ...
        const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, cellSize * 0.5);
        grad.addColorStop(0, PLACEHOLDER_COLORS[i % PLACEHOLDER_COLORS.length]);
        grad.addColorStop(1, '#1A0505');
        ctx.fillStyle = grad;
        ctx.fillRect(cx - cellSize / 2, cy - cellSize / 2, cellSize, cellSize);
        // Silhouette icon so disc is visually non-empty
        ctx.fillStyle = 'rgba(255,255,255,0.22)';
        ctx.beginPath();
        ctx.arc(cx, cy - cellSize * 0.07, cellSize * 0.18, 0, Math.PI * 2);
        ctx.fill();
        ctx.beginPath();
        ctx.ellipse(cx, cy + cellSize * 0.28, cellSize * 0.24, cellSize * 0.18, 0, 0, Math.PI * 2);
        ctx.fill();
      }
    };

    // Upload placeholders immediately so discs are visible from frame 1
    drawPlaceholders();
    gl.bindTexture(gl.TEXTURE_2D, this.tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, offscreen);

    // Load real face images and swap in
  ...
  _onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      ...
      if (ni !== this.nearestVertexIndex) {
        this.nearestVertexIndex = ni;
        const itemIndex = ni % Math.max(1, this.items.length);
        this.onActiveItemChange(itemIndex);
      }
      this.control.snapTargetDirection = vec3.normalize(vec3.create(), this._getVertexWorldPosition(ni ?? 0));
    } else {
  ...
  _findNearestVertexIndex() {
    ...
    let maxD = -1, nearestVertexIndex = 0;
...
const defaultItems = [{ image: 'https://picsum.photos/seed/default/400/400', link: '#', title: '', description: '' }];
...
export default function InfiniteMenu({ items = [], scale = 1.0, onActiveItemChange }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      const item = items.length ? items[itemIndex] : defaultItems[0];
      setActiveItem(item);
      if (onActiveItemChange) onActiveItemChange(itemIndex);
    };
    ...
    // ResizeObserver catches container visibility changes (e.g. AOS fade-in, tab switch)
    const ro = new ResizeObserver(() => { if (sketchRef.current) sketchRef.current.resize(); });
    ro.observe(canvas);

    // Kick off initial resize after a frame so the container has resolved its dimensions
